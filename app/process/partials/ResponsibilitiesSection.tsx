      ...
      <div className={shell}>
        ...
        <div className="mt-14 grid gap-0 md:grid-cols-2">
          {clientResponsibilities.map((item, index) => (
            <article
              className={`grid gap-2 border-b border-line py-7 md:min-h-30 ${
                index % 2 === 1 ? "md:border-l md:pl-30" : "md:pr-0"
              }`}
              data-animate-item
              key={item.label}
            >
              ...
              <p className="max-w-120 text-[15px] leading-[1.5] text-muted">
                {item.body}
              </p>
            </article>
          ))}
