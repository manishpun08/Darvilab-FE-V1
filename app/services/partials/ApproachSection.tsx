...
import { useApproachStack } from "@/hooks/useFrameScroll";
    ...
    >
      <div className={shell}>
        <div className="grid gap-7 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] lg:items-end lg:gap-[clamp(48px,9vw,120px)]">
          ...
          <p className="w-full lg:ml-auto lg:max-w-[59ch] text-[16px] leading-[1.8] text-muted">
        ...
        <div ref={containerRef} className="mt-14 flex flex-col gap-4">
          {service.phases.map((phase, index) => {
            return (
              <article
                className="grid gap-8 rounded-2xl border border-[rgba(7,16,43,0.06)] bg-white p-8 shadow-[0_2px_20px_rgba(7,16,43,0.06)] lg:grid-cols-[minmax(220px,0.28fr)_minmax(0,0.72fr)] lg:gap-14 lg:p-10"
                key={phase.name}
                style={{
                  position: "sticky",
                  top: `${index * 24}px`,
              ...
              >
                <div className="grid content-start gap-2">
                  <span className="font-case text-[clamp(4.6rem,9vw,8rem)] font-semibold leading-[0.82] tracking-[-0.09em] text-[rgba(7,16,43,0.08)]">
