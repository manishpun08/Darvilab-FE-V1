...
function createObserver(el: Element, callback: () => void, threshold = 0.2) {
...
export function useSelectedWorkReveal(
  ...
  useEffect(() => {
    ...
    items.forEach((item) => {
      ...
      if (visual)
        tl.to(visual, { opacity: 1, x: 0, scale: 1, filter: "blur(0px)" });
        ...
        triggers.push(
          ScrollTrigger.create({
            ...
            onEnter: () => {
              ...
              if (badge)
                tl.to(badge, { opacity: 1, y: 0, duration: 0.5 }, "-=0.5");
              if (title)
                tl.to(title, { opacity: 1, y: 0, duration: 0.6 }, "-=0.45");
              if (body)
                tl.to(body, { opacity: 1, y: 0, duration: 0.55 }, "-=0.4");
            },
          }),
        );
      });
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          if (text)
            tl.to(
              text,
              { opacity: 1, letterSpacing: "0.14em", duration: 0.8 },
              "-=0.5",
            );
        },
      });
  ...
  useEffect(() => {
    ...
    if (foundersLabel) {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          if (text)
            tl.to(
              text,
              { opacity: 1, letterSpacing: "0.14em", duration: 0.7 },
              "-=0.5",
            );
        },
      });

      gsap.set(foundersLabel, { y: 0 });
      ...
      ScrollTrigger.create({
        ...
        scrub: 1.5,
        onUpdate(self) {
          gsap.set(foundersLabel, { y: self.progress * -30 });
        },
      });
    }

    // 2. Founder Statements
    const founderItems = el.querySelectorAll<HTMLElement>(
    ...
    founderItems.forEach((item, index) => {
      ...
      if (avatar)
        gsap.set(avatar, { opacity: 0, x: alignRight ? 25 : -25, scale: 0.94 });
    ...
    const specialistsLabel = el.querySelector(
    ...
    const specialists = el.querySelectorAll<HTMLElement>(
    ...
    if (specialists.length) {
      specialists.forEach((spec, i) => {
        ...
        gsap.set(spec, { opacity: 0, y: 20, filter: "blur(3px)" });
        if (avatarImg) gsap.set(avatarImg, { scale: 0.9, opacity: 0 });
...
export function useProofSectionReveal(
  ...
  useEffect(() => {
    ...
    const metricCards = el.querySelectorAll<HTMLElement>(
      "[data-animate-metric-card]",
    );

    if (tagline) gsap.set(tagline, { opacity: 0, x: -20 });
    if (titleSpans.length)
      gsap.set(titleSpans, { opacity: 0, y: 40, filter: "blur(5px)" });
    ...
    if (metricCards.length) {
      metricCards.forEach((card, i) => {
        const valueEl = card.querySelector<HTMLElement>(
...
export function useWhereWeWorkReveal(
  ...
  useEffect(() => {
    ...
    if (mapContainer)
      gsap.set(mapContainer, { opacity: 0, scale: 0.96, filter: "blur(6px)" });
